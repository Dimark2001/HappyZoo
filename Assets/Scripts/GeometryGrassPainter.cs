...
public class GeometryGrassPainter : MonoBehaviour
{
    ...
    void OnScene(SceneView scene)
    {
        ...
        if ((Selection.Contains(gameObject)))
        {
            ...
            if (e.type == EventType.MouseDrag && e.button == 1 && toolbarInt == 2)
            {
                ...
                if (Physics.Raycast(ray, out terrainHit, 200f, hitMask.value))
                {
                    var texture = (Texture2D)terrainHit.collider.GetComponent<MeshRenderer>().material.mainTexture;
                    hitPos = terrainHit.point;
                    hitPosGizmo = hitPos;
                    hitNormal = terrainHit.normal;
                    for (int j = 0; j < positions.Count; j++)
                    {
                        ...
                        float dist = Vector3.Distance(terrainHit.point, pos+ this.transform.position);

                        // if its within the radius of the brush, remove all info
                        if (dist <= brushSize)
                        {
                            if(Physics.Raycast(pos + this.transform.position + new Vector3(0,0.5f, 0), new Vector3(0,-1,0),  out RaycastHit pointHit, 2f))
                         
                            colors[j] = (texture.GetPixel((int)(pointHit.textureCoord.x*texture.width), (int)(pointHit.textureCoord.y*texture.height)));
    ...
    public void ReadMehInfo()
    {
        filter = GetComponent<MeshFilter>();
        mesh = filter.mesh;
        positions = mesh.vertices.ToList();
        indi = mesh.GetIndices(0);
        length = mesh.uv.ToList();
